...
**Pre-run validation hooks.** `[pipeline] pre_run = ["module.path:callable", ...]` declares callables that run **before any materialization** on every dagnet launch path — plain `run`, `--select`, and `--from-failure`. Each receives a `PreRunContext` (the manifest path, the **effective store root**, the run preset's name, the selection resolved to concrete node names and asset keys — or "everything" — the resolved run config, and `is_resume`/`parent_run_id`) and either returns a `Diagnostics` or raises. `store_root` is the effective one — the `--store-root` override when given, else `[pipeline] store_root`, else the default, always absolute — and hooks must use it rather than re-deriving locations from the manifest. Under `--store-root` the two disagree, and from inside a hook nothing in the manifest reveals which is live; a hook that resolves durable locations itself would act on a different database from the one the run writes to. For a hook that deletes, that is silent wrong-database deletion under a documented flag. The field has no default for the same reason: a plausible-looking wrong location is the exact failure it closes.

`is_resume` matters more than it looks: a resume re-executes a *subset* of the selection, so it presents exactly like the narrow selection a guard exists to refuse, and anything that clears state before writing must not clear it again for steps that already succeeded. The resume target is resolved before the hooks run, which is why launching `--from-failure` with nothing to resume is a usage error rather than something a hook sees. Any ERROR-severity diagnostic aborts the launch with the same aggregated, located output `dagnet check` produces; WARNING prints and the run proceeds. A hook that raises becomes an ERROR naming it, so a broken guard cannot let a launch through. Every hook runs even after one objects, so a person sees all objections at once. Absent or empty means no hooks and no behaviour change. Hooks are named in the map like everything else, so a reader can see what guards a pipeline without reading any Python.
...
- [P] **`[pipeline] pre_execute` setup hooks** (§8) — added 2026-07-27, ruling on dagnet-db's "who clears the tables" question. The answer is a *second hook kind*, not a widening of `pre_run`: the two differ on side effects, and the aggregate-vs-stop-at-first-failure behaviour follows from that difference rather than being an independent choice. Clearing tables belongs on the committed side of the gate, so a refused launch never destroys anything.
  ...
  - [P] **`store_root` on the hook context** — added 2026-07-27. Without it a hook resolving durable locations from the manifest would, under `dagnet run --store-root <elsewhere>`, act on a different place from the one the run writes to, indistinguishably from inside the hook: silent wrong-database deletion under a documented flag. Fixed on report rather than waiting for a consumer to hit it. Populated by the same call the compiler makes, so hook and run cannot disagree.
...
- [P] **`resolve_artifacts` and `store_root` are public API** — added 2026-07-27. dagnet-db needs artifact locations and declined to reimplement the resolution; a second copy of "where does this pipeline keep its things" is the shadow path registry `[artifacts]` exists to abolish, so the dependency is explicit instead. Removing either from `dagnet.__all__` is a breaking change, and `tests/test_public_api.py` says so.
